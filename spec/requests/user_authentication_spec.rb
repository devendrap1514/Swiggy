...
RSpec.describe "UserAuthentications", type: :request do
  ...
  describe "POST /forgot_password" do
    it "return success" do
      post '/auth/send_mail', params: { username: user.username }
      expect(response).to have_http_status(:ok) # 200
    end
    it "return not_found userame" do
      post '/auth/send_mail', params: { username: "--d--" }
  ...
  describe "POST /reset_password" do
    it "return success" do
      post '/auth/set_password', params: { token: user.reset_password_token, password: "Ugh123" }
      expect(response).to have_http_status(:ok) # 200
    end
    it "return not_found token" do
      post '/auth/set_password', params: { token: nil, password: "Ugh123" }
      expect(response).to have_http_status(:not_found) # 404
    end
    it "return unprocessable_entity" do
      post '/auth/set_password', params: { token: user.reset_password_token, password: "Ugh" }
      expect(response).to have_http_status(:unprocessable_entity) # 422
    end
    it "return expire token" do
      post '/auth/set_password', params: { token: '0000', password: "Ugh123" }
